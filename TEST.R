...
for (k in 1:length(n[1:maxn])) {
  print(k)
  resih<-simulvarmanoidih(NV = n[k],simulazioni = 1000)
...
for (k in 1:length(n[1:4])) {
  print(k)
  resih<-simulvarmanoidnet(NV = n[k],simulazioni = 1000)
  diff_irf[k]<-resih$diff_irf
  irf1c<-model$irf1[,,-1]
  resih$irf2<-resih$irf2[-(1:(w*w)),]
  irf2[[k]]<-resih$irf2
...
#VARIANZA
resvarma<-cbind(diff_irf, biases, VARIANZA)
saveRDS(resvarma, file = "tabellaNET.RDS")
kable(risultati_varma1[[c(1,2)]]$ar, format = "latex")
saveRDS(risultati_varma1, file = "risultatiNET.RDS")
saveRDS(irf2, file = "irf2NET.RDS")
risultati_varma1[[1000]]
dim(irf2[[1]])
varma1<-rvarma(3,1,1)
#predict.varma(varma1, 10)
...
for (k in 1:length(n[1:4])) {
  print(k)
  resih<-simulvarmanoidfkf(NV = n[k],simulazioni = 500)
  diff_irf[k]<-resih$diff_irf
  irf1c<-model$irf1[,,-1]
  resih$irf2<-resih$irf2[-(1:(w*w)),]
  irf2[[k]]<-resih$irf2
...
#VARIANZA
resvarma<-cbind(diff_irf, biases, VARIANZA)
saveRDS(resvarma, file = "tabellaFKF.RDS")
#kable(risultati_varma1[[c(1,2)]]$ar, format = "latex")
saveRDS(risultati_varma1, file = "risultatiFKF.RDS")
saveRDS(irf2, file = "irf2FKF.RDS")
risultati_varma1[[1000]]
dim(irf2[[1]])
varma1<-rvarma(3,1,1)
#predict.varma(varma1, 10)
...
for (k in 1:length(n[1:4])) {
  print(k)
  resih<-simulvarmanoidkfas(NV = n[k],simulazioni = 500)
  diff_irf[k]<-resih$diff_irf
  irf1c<-model$irf1[,,-1]
  resih$irf2<-resih$irf2[-(1:(w*w)),]
  irf2[[k]]<-resih$irf2
